        ...
        if os.path.exists(filepath):
            try:
                with open(filepath, 'r') as f:
                    ...
                    print(f"✅ Loaded {len(self.sent_whale_tx_hashes)} whale alert hashes")
            except Exception as e:
                print(f"⚠️  Error loading whale hashes: {e}")
    
    def _save_whale_hashes(self):
        """Save whale tx hashes to file for persistence"""
        filepath = self._get_whale_hashes_file()
        try:
            with open(filepath, 'w') as f:
                json.dump({'hashes': list(self.sent_whale_tx_hashes)}, f)
        except Exception as e:
            print(f"⚠️  Error saving whale hashes: {e}")
